## Kwe Chain
Kwe chain is written in [Rust](https://www.rust-lang.org/). A basic familiarity with Rust tooling is required.

### Clone
To clone the repo with its submodules run:
```bash
git clone https://github.com/krypto-wealth
```
...
```bash
./target/release/krypto_wealth-node --dev
```

Purge the development chain's state:

```bash
./target/release/krypto_wealth-node purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_LOG=debug RUST_BACKTRACE=1 ./target/release/krypto_wealth-node -lruntime=debug --dev
```
...
```
./target/release/krypto_wealth-node benchmark \
    --chain=dev \
    --steps=50 \
    --repeat=20 \
    --pallet=module_poc \
...
```bash
./target/release/krypto_wealth-node -h
```
