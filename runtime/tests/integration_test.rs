...
fn test_evm_module() {
	ExtBuilder::default()
		.balances(vec![
			(alice_account_id(), CurrencyId::Token(TokenSymbol::KWE), amount(1 * MILLI_KWE)),
			(bob_account_id(), CurrencyId::Token(TokenSymbol::KWE), amount(1 * MILLI_KWE)),
		])
		.build()
		.execute_with(|| {
			assert_eq!(Balances::free_balance(alice_account_id()), amount(1 * MILLI_KWE));
			assert_eq!(Balances::free_balance(bob_account_id()), amount(1 * MILLI_KWE));
			...
			assert_eq!(Balances::free_balance(bob_account_id()), amount(1 * MILLI_KWE) - amount(10 * MICRO_KWE));
		});
}
