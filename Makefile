...
.PHONY: wasm
wasm:
	cargo build -p krypto_wealth-runtime --features with-ethereum-compatibility --release

.PHONY: genesis
genesis:
	make release
	./target/release/krypto_wealth-node build-spec --chain testnet-new > assets/chain_spec_testnet.json
	./target/release/krypto_wealth-node build-spec --chain mainnet-new > assets/chain_spec_mainnet.json
	./target/release/krypto_wealth-node build-spec --chain testnet-new --raw > assets/chain_spec_testnet_raw.json
	./target/release/krypto_wealth-node build-spec --chain mainnet-new --raw > assets/chain_spec_mainnet_raw.json
...
.PHONY: debug
debug:
	cargo build && RUST_LOG=debug RUST_BACKTRACE=1 rust-gdb --args target/debug/krypto_wealth-node --dev --tmp -lruntime=debug
...
ifeq (,$(wildcard fork/data))
	...
	cp target/release/krypto_wealth-node fork/data/binary
	cp target/release/wbuild/krypto_wealth-runtime/krypto_wealth_runtime.compact.wasm fork/data/runtime.wasm
