...
use krypto_wealth_primitives::{AccountPublic, Balance, Nonce};
use krypto_wealth_runtime::BABE_GENESIS_EPOCH_CONFIG;
...
pub struct Extensions {
	...
	pub fork_blocks: sc_client_api::ForkBlocks<krypto_wealth_primitives::Block>,
	/// Known bad block hashes.
	pub bad_blocks: sc_client_api::BadBlocks<krypto_wealth_primitives::Block>,
}
...
pub fn local_testnet_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		...
		None,
		// Protocol ID
		Some("krypto_wealth_local_testnet"),
		// Properties
		Some(krypto_wealth_properties()),
		// Extensions
		Default::default(),
...
pub fn public_testnet_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		...
		// Bootnodes
		vec!["/dns/bootnode-t1.krypto_wealthscan.com/tcp/30334/p2p/12D3KooWKmFtS7BFtkkKWrP5ZcCpPFokmST2JFXFSsVBNeW5SXWg".parse().unwrap()],
		// Telemetry
		TelemetryEndpoints::new(vec![(TELEMETRY_URL.into(), 0)]).ok(),
		// Protocol ID
		Some("krypto_wealth_testnet"),
		// Properties
		Some(krypto_wealth_properties()),
		// Extensions
		Default::default(),
...
pub fn mainnet_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		...
		// Bootnodes
		vec![
			"/dns/bootnode.krypto_wealthscan.com/tcp/30333/p2p/12D3KooWFHSc9cUcyNtavUkLg4VBAeBnYNgy713BnovUa9WNY5pp".parse().unwrap(),
			"/dns/bootnode.krypto_wealth.finance/tcp/30333/p2p/12D3KooWAQqcXvcvt4eVEgogpDLAdGWgR5bY1drew44We6FfJAYq".parse().unwrap(),
			"/dns/bootnode.krypto_wealth-chain.com/tcp/30333/p2p/12D3KooWCT7rnUmEK7anTp7svwr4GTs6k3XXnSjmgTcNvdzWzgWU".parse().unwrap(),
		],
		// Telemetry
		TelemetryEndpoints::new(vec![(TELEMETRY_URL.into(), 0)]).ok(),
		// Protocol ID
		Some("krypto_wealth_mainnet"),
		// Properties
		Some(krypto_wealth_properties()),
		// Extensions
		Default::default(),
...
fn testnet_genesis(
	...
	const INITIAL_BALANCE: u128 = 100_000_000 * KWE;
	const INITIAL_STAKING: u128 =   1_000_000 * KWE;
...
fn mainnet_genesis(
	...
	let balances = initial_authorities
		...
		.chain(endowed_accounts.iter().cloned().map(|x| (x.0.clone(), x.1 * KWE)))
		.chain(
			get_all_module_accounts()
				.iter()
				.map(|x| (x.clone(), existential_deposit)),
...
/// Token
pub fn krypto_wealth_properties() -> Properties {
	let mut p = Properties::new();
	p.insert("ss58format".into(), 42.into());
	p.insert("tokenDecimals".into(), 18.into());
	p.insert("tokenSymbol".into(), "KWE".into());
	p
}
