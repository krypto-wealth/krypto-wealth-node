...
fn charges_fee() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Currencies::free_balance(KWE, &ALICE), (100000 - fee).into());
		...
		assert_eq!(
			Currencies::free_balance(KWE, &ALICE),
...
fn charges_fee_when_pre_dispatch_and_native_currency_is_enough() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert!(ChargeTransactionPayment::<Runtime>::from(0)
			...
			.is_ok());
		assert_eq!(Currencies::free_balance(KWE, &ALICE), 100000 - fee);
	});
}
...
fn refund_fee_according_to_actual_when_post_dispatch_and_native_currency_is_enough() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Currencies::free_balance(KWE, &ALICE), 100000 - fee + refund);
	});
}
...
fn charges_fee_when_validate_and_native_is_not_enough() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(KWE, &BOB), 0);
		...
		// 	false
		// ));
		// assert_eq!(DEXModule::get_liquidity_pool(KWE, RUSD), (10000, 1000));
		...
		// assert_eq!(Currencies::free_balance(KWE, &BOB), 0);
		// assert_eq!(Currencies::free_balance(RUSD, &BOB), 749);
		// assert_eq!(DEXModule::get_liquidity_pool(KWE, RUSD), (10000 - 2000, 1251));
	});
}
