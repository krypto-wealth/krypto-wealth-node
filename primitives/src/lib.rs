...
pub mod currency {
	...
	pub const KWE: Balance = DOLLARS;
	pub const MILLI_KWE: Balance = KWE / 1_000;
	pub const MICRO_KWE: Balance = KWE / 1_000_000;
	
	pub const fn deposit(items: u32, bytes: u32) -> Balance {
		items as Balance * 20 * KWE + (bytes as Balance) * 100 * MILLI_KWE
	}
}
